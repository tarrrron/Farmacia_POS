<div class="page-title">
    ...
    <p>El nuevo usuario se creará con estado activo.</p>
</div>
...
<section class="panel">
    ...
    <?php endif; ?>
    <?php if (!$profiles): ?>
        <p role="status">No hay perfiles activos disponibles. Activa o crea un perfil antes de registrar usuarios.</p>
    <?php endif; ?>
    <form method="post" action="index.php?route=module/usuarios/guardar">
        ...
        <select id="idperfil" name="idperfil" required>
            ...
            <?php foreach ($profiles as $profile): ?>
                <option value="<?= (int) $profile["idperfil"] ?>" <?= (string) $profile["idperfil"] === $user["idperfil"] ? "selected" : "" ?>><?= htmlspecialchars($profile["nombre"], ENT_QUOTES, "UTF-8") ?></option>
            <?php endforeach; ?>
        </select>
        <button class="btn btn-primary" type="submit" <?= !$profiles ? "disabled" : "" ?>>Guardar usuario</button>
